...
# Root
[[ $(id -u) != 0 ]] && echo -e "11 哎呀……请使用 ${red}root ${none}用户运行 ${yellow}~(^_^) ${none}" && exit 1
...
if [[ $sys_bit == "i386" || $sys_bit == "i686" ]]; then
	...
	echo -e "24 哈哈……这个 ${red}辣鸡脚本${none} 不支持你的系统。 ${yellow}(-_-) ${none}" && exit 1
fi
...
if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
	...
	echo -e "44 哎呀哎呀…… ${red}出错咯...请重新安装V2Ray${none} ${yellow}~(^_^) ${none}" && exit 1
fi
...
if [[ ! -f $_v2ray_sh ]]; then
	...
	chmod +x $_v2ray_sh
	echo -e "\n $yellow 76 警告: 请重新登录 SSH 以避免出现 v2ray 命令未找到的情况。$none  \n" && exit 1
fi

if [ $v2ray_pid ]; then
	v2ray_status="$green 80正在运行$none"
else
	v2ray_status="$red 82未在运行$none"
fi
if [[ $v2ray_transport == [45] && $caddy ]] && [[ $caddy_pid ]]; then
	caddy_run_status="$green 85正在运行$none"
else
	caddy_run_status="$red 87未在运行$none"
fi
...
change_v2ray_config() {
	local _menu=(
		"631修改 V2Ray 端口"
		"632修改 V2Ray 传输协议"
		"633修改 V2Ray 动态端口 (如果可以)"
		"634修改 用户ID ( UUID )"
		"635修改 TLS 域名 (如果可以)"
		"636修改 分流的路径 (如果可以)"
		"637修改 伪装的网址 (如果可以)"
		"638关闭 网站伪装 和 路径分流 (如果可以)"
		"639开启 / 关闭 广告拦截"
	)
	while :; do
		...
		echo
		read -p "$(echo -e "652请选择 [${magenta}1-${#_menu[*]}$none]:")" _opt
		if [[ -z $_opt ]]; then
			error
		else
			case $_opt in
...
change_v2ray_port() {
	if [[ $v2ray_transport == 4 ]]; then
		echo
		echo -e " 711由于你目前使用的是$yellow WebSocket + TLS $none传输协议...所以修不修改 V2Ray 端口没有什么不一样的"
		echo
		echo " 713如果你想要使用其他端口...可以先修改 V2Ray 的传输协议..之后再修改 V2Ray 端口"
		echo
		change_v2ray_transport_ask
	elif [[ $v2ray_transport == 5 ]]; then
		echo
		echo -e " 718由于你目前使用的是$yellow HTTP/2 $none传输协议...所以修不修改 V2Ray 端口没有什么不一样的"
		echo
		echo " 720如果你想要使用其他端口...可以先修改 V2Ray 的传输协议..之后再修改 V2Ray 端口"
		echo
		change_v2ray_transport_ask
	else
		echo
		while :; do
			echo -e "726请输入 "$yellow"V2Ray"$none" 端口 ["$magenta"1-65535"$none"]"
			read -p "$(echo -e "(当前端口: ${cyan}${v2ray_port}$none):")" v2ray_port_opt
			[[ -z $v2ray_port_opt ]] && error && continue
			case $v2ray_port_opt in
			$v2ray_port)
				echo
				echo " 732哎呀...跟当前端口一毛一样呀...修改个鸡鸡哦"
				error
				;;
			[1-9] | [1-9][0-9] | [1-9][0-9][0-9] | [1-9][0-9][0-9][0-9] | [1-5][0-9][0-9][0-9][0-9] | 6[0-4][0-9][0-9][0-9] | 65[0-4][0-9][0-9] | 655[0-3][0-5])
				if [[ $dynamicPort ]] && [[ $v2ray_dynamicPort_start == $v2ray_port_opt || $v2ray_dynamicPort_end == $v2ray_port_opt ]]; then
					echo
					echo -e "738 抱歉，此端口和 V2Ray 动态端口 冲突，当前 V2Ray 动态端口范围为：${cyan}$port_range${none}"
					error
				elif [[ $dynamicPort ]] && [[ $v2ray_dynamicPort_start -lt $v2ray_port_opt && $v2ray_port_opt -le $v2ray_dynamicPort_end ]]; then
					echo
					echo -e " 742 抱歉，此端口和 V2Ray 动态端口 冲突，当前 V2Ray 动态端口范围为：${cyan}$port_range${none}"
					error
				elif [[ $shadowsocks && $v2ray_port_opt == $ssport ]]; then
					echo
					echo -e "746 抱歉, 此端口跟 Shadowsocks 端口冲突...当前 Shadowsocks 端口: ${cyan}$ssport$none"
					error
				elif [[ $socks && $v2ray_port_opt == $socks_port ]]; then
					echo
					echo -e "750 抱歉, 此端口跟 Socks 端口冲突...当前 Socks 端口: ${cyan}$socks_port$none"
					error
				elif [[ $mtproto && $v2ray_port_opt == $mtproto_port ]]; then
					echo
					echo -e "754 抱歉, 此端口跟 MTProto 端口冲突...当前 MTProto 端口: ${cyan}$mtproto_port$none"
					error
				else
					echo
					echo
					echo -e "$yellow 759 V2Ray 端口 = $cyan$v2ray_port_opt$none"
					echo "----------------------------------------------------------------"
					echo
					pause
...
download_v2ray_config_ask() {
	echo
	while :; do
		echo -e "786 是否需要 下载 V2Ray 配置 / 生成配置信息链接 / 生成二维码链接 [${magenta}Y/N$none]"
		read -p "$(echo -e "787 默认 [${cyan}N$none]:")" y_n
		[ -z $y_n ] && y_n="n"
		if [[ $y_n == [Yy] ]]; then
...
change_v2ray_transport_ask() {
	echo
	while :; do
		echo -e "803 是否需要修改$yellow V2Ray $none传输协议 [${magenta}Y/N$none]"
		read -p "$(echo -e "804 默认 [${cyan}N$none]:")" y_n
		[ -z $y_n ] && break
		if [[ $y_n == [Yy] ]]; then
			change_v2ray_transport
			break
...
change_v2ray_transport() {
	echo
	while :; do
		echo -e "819 请选择 "$yellow"V2Ray"$none" 传输协议 [${magenta}1-${#transport[*]}$none]"
		echo
		...
		echo
		echo "832 备注1: 含有 [dynamicPort] 的即启用动态端口.."
		echo "833 备注2: [utp | srtp | wechat-video | dtls | wireguard] 分别伪装成 [BT下载 | 视频通话 | 微信视频通话 | DTLS 1.2 数据包 | WireGuard 数据包]"
		echo
		read -p "$(echo -e "(835 当前传输协议: ${cyan}${transport[$v2ray_transport - 1]}$none)"):" v2ray_transport_opt
		if [ -z "$v2ray_transport_opt" ]; then
			...
			$v2ray_transport)
				echo
				echo " 842 哎呀...跟当前传输协议一毛一样呀...修改个鸡鸡哦"
				error
				;;
			4 | 5)
				if [[ $v2ray_port == "80" || $v2ray_port == "443" ]]; then
					echo
					echo -e " 848 抱歉...如果你想要使用${cyan} ${transport[$v2ray_transport_opt - 1]} $none传输协议.. ${red}V2Ray 端口不能为 80 或者 443 ...$none"
					echo
					echo -e " 850 当前 V2Ray 端口: ${cyan}$v2ray_port$none"
					error
				elif [[ $shadowsocks ]] && [[ $ssport == "80" || $ssport == "443" ]]; then
					echo
					echo -e " 854 抱歉...如果你想要使用${cyan} ${transport[$v2ray_transport_opt - 1]} $none传输协议.. ${red}Shadowsocks 端口不能为 80 或者 443 ...$none"
					echo
					echo -e " 856 当前 Shadowsocks 端口: ${cyan}$ssport$none"
					error
				elif [[ $socks ]] && [[ $socks_port == "80" || $socks_port == "443" ]]; then
					echo
					echo -e " 860 抱歉...如果你想要使用${cyan} ${transport[$v2ray_transport_opt - 1]} $none传输协议.. ${red}Socks 端口不能为 80 或者 443 ...$none"
					echo
					echo -e " 862 当前 Socks 端口: ${cyan}$socks_port$none"
					error
				elif [[ $mtproto ]] && [[ $mtproto_port == "80" || $mtproto_port == "443" ]]; then
					echo
					echo -e " 866 抱歉...如果你想要使用${cyan} ${transport[$v2ray_transport_opt - 1]} $none传输协议.. ${red}MTProto 端口不能为 80 或者 443 ...$none"
					echo
					echo -e " 868 当前 MTProto 端口: ${cyan}$mtproto_port$none"
					error
				else
					echo
					echo
					echo -e "$yellow 873 V2Ray 传输协议 = $cyan${transport[$v2ray_transport_opt - 1]}$none"
					echo "----------------------------------------------------------------"
					echo
					break
				fi
			...
			[1-9] | [1-2][0-9] | 3[0-2])
				echo
				echo
				echo -e "$yellow 882 V2Ray 传输协议 = $cyan${transport[$v2ray_transport_opt - 1]}$none"
				echo "----------------------------------------------------------------"
				echo
				break
				;;
...
tls_config() {
	while :; do
		...
		echo
		echo
		echo -e "953 请输入一个 $magenta正确的域名$none，一定一定一定要正确，不！能！出！错！"
		read -p "954 (例如：233blog.com): " new_domain
		[ -z "$new_domain" ] && error && continue
		echo
		echo
		echo -e "$yellow 958 你的域名 = $cyan$new_domain$none"
		echo "----------------------------------------------------------------"
		break
	done
	get_ip
	echo
	echo
	echo -e "$yellow 965 请将 $magenta$new_domain$none $yellow解析到: $cyan$ip$none"
	echo
	echo -e "$yellow 967 请将 $magenta$new_domain$none $yellow解析到: $cyan$ip$none"
	echo
	echo -e "$yellow 969 请将 $magenta$new_domain$none $yellow解析到: $cyan$ip$none"
	echo "----------------------------------------------------------------"
	echo

	while :; do

		read -p "$(echo -e "(975 是否已经正确解析: [${magenta}Y$none]):") " record
		if [[ -z "$record" ]]; then
			error
		else
			if [[ "$record" == [Yy] ]]; then
				...
				echo
				echo
				echo -e "$yellow 983 域名解析 = ${cyan}我确定已经有解析了$none"
				echo "----------------------------------------------------------------"
				echo
				break
			else
...
auto_tls_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(1072 是否自动配置 TLS: [${magenta}Y/N$none]):") " auto_install_caddy
		if [[ -z "$auto_install_caddy" ]]; then
			...
			if [[ "$auto_install_caddy" == [Yy] ]]; then
				echo
				echo
				echo -e "$yellow 1079 自动配置 TLS = $cyan打开$none"
				echo "----------------------------------------------------------------"
				echo
				path_config_ask
				pause
				...
				echo
				echo
				echo -e "$yellow 1110 自动配置 TLS = $cyan关闭$none"
				echo "----------------------------------------------------------------"
				echo
				pause
				domain_check
...
path_config_ask() {
	echo
	while :; do
		echo -e "1139 是否开启 网站伪装 和 路径分流 [${magenta}Y/N$none]"
		read -p "$(echo -e "(1140 默认: [${cyan}N$none]):")" path_ask
		...
		N | n)
			echo
			echo
			echo -e "$yellow 1151 网站伪装 和 路径分流 = $cyan不想配置$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
path_config() {
	echo
	while :; do
		echo -e "1165 请输入想要 ${magenta}用来分流的路径$none , 例如 /233blog , 那么只需要输入 233blog 即可"
		read -p "$(echo -e "(1166 默认: [${cyan}233blog$none]):")" new_path
		[[ -z $new_path ]] && new_path="233blog"

		case $new_path in
		*[/$]*)
			echo
			echo -e " 1172 由于这个脚本太辣鸡了..所以分流的路径不能包含$red / $none或$red $ $none这两个符号.... "
			echo
			error
			;;
		*)
			echo
			echo
			echo -e "$yellow 1179 分流的路径 = ${cyan}/${new_path}$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
proxy_site_config() {
	echo
	while :; do
		echo -e "1191 请输入 ${magenta}一个正确的$none ${cyan}网址$none 用来作为 ${cyan}网站的伪装$none , 例如 https://liyafly.com"
		...
		*[#$]*)
			echo
			echo -e " 1202 由于这个脚本太辣鸡了..所以伪装的网址不能包含$red # $none或$red $ $none这两个符号.... "
			echo
			error
			;;
		*)
			echo
			echo
			echo -e "$yellow 1209 伪装的网址 = ${cyan}${new_proxy_site}$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
change_v2ray_id() {
	echo
	while :; do
		echo -e "1507 是否确定要修改用户ID [${magenta}Y/N$none]"
		read -p "$(echo -e "默认 [${cyan}N$none]:")" y_n
		if [[ -z $y_n ]]; then
			echo
			echo -e "$green 1511 已取消修改用户ID...$none"
			echo
			break
		else
			if [[ $y_n == [Yy] ]]; then
				echo
				echo
				echo -e "$yellow 1518 修改用户ID = $cyan确定$none"
				echo "----------------------------------------------------------------"
				echo
				pause
				backup_config uuid
				...
				echo
				echo -e "$green 1531 已取消修改用户ID...$none"
				echo
				break
			else
				error
...
change_domain() {
	if [[ $v2ray_transport == [45] ]] && [[ $caddy ]]; then
		while :; do
			echo
			echo -e "1544 请输入一个 $magenta正确的域名$none，一定一定一定要正确，不！能！出！错！"
			read -p "$(echo -e "(当前域名: ${cyan}$domain$none):") " new_domain
			[ -z "$new_domain" ] && error && continue
			if [[ $new_domain == $domain ]]; then
				echo
				echo -e "1549 跟当前域名一毛一样啊...修改个鸡鸡哦"
				echo
				error && continue
			fi
			echo
			echo
			echo -e "$yellow 1555 你的域名 = $cyan$new_domain$none"
			echo "----------------------------------------------------------------"
			break
		done
		get_ip
		...
		while :; do
			...
			read -p "$(echo -e "( 1572是否已经正确解析: [${magenta}Y$none]):") " record
			if [[ -z "$record" ]]; then
				error
			else
				if [[ "$record" == [Yy] ]]; then
					...
					echo
					echo
					echo -e "$yellow 1580 域名解析 = ${cyan}我确定已经有解析了$none"
					echo "----------------------------------------------------------------"
					echo
					pause
					# domain_check
		...
		echo
		echo -e "$red 1601 抱歉...不支持修改...$none"
		echo
		echo -e " 1603 备注..修改 TLS 域名仅支持传输协议为 ${yellow}WebSocket + TLS$none 或 ${yellow}HTTP/2$none 并且$yellow 自动配置 TLS = 打开$none"
		echo
		echo -e " 1605 当前传输协议为: ${cyan}${transport[$v2ray_transport - 1]}${none}"
		echo
		if [[ $caddy ]]; then
			echo -e " 1608 自动配置 TLS = ${cyan}打开$none"
		else
			echo -e "1610 自动配置 TLS = $red关闭$none"
		fi
		echo
	fi
}
change_path_config() {
	if [[ $v2ray_transport == [45] ]] && [[ $caddy && $is_path ]]; then
		echo
		while :; do
			echo -e "1619请输入想要 ${magenta}用来分流的路径$none , 例如 /233blog , 那么只需要输入 233blog 即可"
			read -p "$(echo -e "(1620当前分流的路径: [${cyan}/${path}$none]):")" new_path
			[[ -z $new_path ]] && error && continue

			case $new_path in
			$path)
				echo
				echo -e " 1626 大佬...跟 当前分流的路径 一毛一样啊...修改个鸡鸡哦 "
				echo
				error
				;;
			*[/$]*)
				echo
				echo -e " 1632 由于这个脚本太辣鸡了..所以分流的路径不能包含$red / $none或$red $ $none这两个符号.... "
				echo
				error
				;;
			*)
				echo
				echo
				echo -e "$yellow 1639 分流的路径 = ${cyan}/${new_path}$none"
				echo "----------------------------------------------------------------"
				echo
				break
				;;
		...
		echo
		echo -e "$red 1675 抱歉...不支持修改...$none"
		echo
		echo -e " 1677 备注..修改 分流的路径 仅支持传输协议为 ${yellow}WebSocket + TLS$none 或 ${yellow}HTTP/2$none 并且$yellow 自动配置 TLS = 打开$none"
		echo
		echo -e " 1679 当前传输协议为: ${cyan}${transport[$v2ray_transport - 1]}${none}"
		echo
		if [[ $caddy ]]; then
			echo -e " 1682 自动配置 TLS = ${cyan}打开$none"
		else
			echo -e " 1684 自动配置 TLS = $red关闭$none"
		fi
		echo
		change_v2ray_transport_ask
	fi
...
change_proxy_site_config() {
	if [[ $v2ray_transport == [45] ]] && [[ $caddy && $is_path ]]; then
		echo
		while :; do
			echo -e "1695 请输入 ${magenta}一个正确的$none ${cyan}网址$none 用来作为 ${cyan}网站的伪装$none , 例如 https://liyafly.com"
			...
			*[#$]*)
				echo
				echo -e " 1706 由于这个脚本太辣鸡了..所以伪装的网址不能包含$red # $none或$red $ $none这两个符号.... "
				echo
				error
				;;
			*)
				echo
				echo
				echo -e "$yellow 1713 伪装的网址 = ${cyan}${new_proxy_site}$none"
				echo "----------------------------------------------------------------"
				echo
				break
				;;
		...
		echo
		echo
		echo " 1726 哎哟...好像是修改成功了..."
		echo
		echo -e " 1728 赶紧打开你的域名 ${cyan}https://${domain}$none 检查一下看看"
		echo
		echo
	elif [[ $v2ray_transport == [45] ]] && [[ $caddy ]]; then
		path_config_ask
		...
		echo
		echo -e "$red 1752 抱歉...不支持修改...$none"
		echo
		echo -e " 1754备注..修改 伪装的网址 仅支持传输协议为 ${yellow}WebSocket + TLS$none 或 ${yellow}HTTP/2$none 并且$yellow 自动配置 TLS = 打开$none"
		echo
		echo -e " 1756当前传输协议为: ${cyan}${transport[$v2ray_transport - 1]}${none}"
		echo
		if [[ $caddy ]]; then
			echo -e " 1759 自动配置 TLS = ${cyan}打开$none"
		else
			echo -e " 1761 自动配置 TLS = $red关闭$none"
		fi
		echo
		change_v2ray_transport_ask
	fi
...
domain_check() {
	...
	if [[ $test_domain != $ip ]]; then
		echo
		echo -e "$red 1773检测域名解析错误....$none"
		echo
		echo -e " 1775 你的域名: $yellow$new_domain$none 未解析到: $cyan$ip$none"
		echo
		echo -e " 1777 你的域名当前解析到: $cyan$test_domain$none"
		echo
		echo " 1779 备注...如果你的域名是使用 Cloudflare 解析的话..在 Status 那里点一下那图标..让它变灰"
		echo
		exit 1
	fi
}
disable_path() {
	if [[ $v2ray_transport == [45] ]] && [[ $caddy && $is_path ]]; then
		...
		while :; do
			echo -e "1789 是否关闭 ${yellow}网站伪装 和 路径分流${none} [${magenta}Y/N$none]"
			read -p "$(echo -e "(默认 [${cyan}N$none]):") " y_n
			[[ -z "$y_n" ]] && y_n="n"
			if [[ "$y_n" == [Yy] ]]; then
				echo
				echo
				echo -e "$yellow 1795 关闭 网站伪装 和 路径分流 = $cyan是$none"
				echo "----------------------------------------------------------------"
				echo
				pause
				backup_config -path
				...
				echo
				echo -e " $green 1809已取消关闭 网站伪装 和 路径分流 ....$none"
				echo
				break
			else
				error
		...
		echo
		echo -e "$red 1819抱歉...不支持修改...$none"
		echo
		echo -e " 1821当前传输协议为: ${cyan}${transport[$v2ray_transport - 1]}${none}"
		echo
		if [[ $caddy ]]; then
			echo -e " 1824 自动配置 TLS = ${cyan}打开$none"
		else
			echo -e "1826自动配置 TLS = $red关闭$none"
		fi
		echo
		if [[ $is_path ]]; then
			echo -e " 1830路径分流 = ${cyan}打开$none"
		else
			echo -e " 1832路径分流 = $red关闭$none"
		fi
		echo
		echo -e " 1835 必须为 WebSocket + TLS 或 HTTP/2 传输协议, 自动配置 TLS = ${cyan}打开$none, 路径分流 = ${cyan}打开$none, 才能修改"
		echo
...
blocked_hosts() {
	if [[ $ban_ad ]]; then
		local _info="$green 1842已开启$none"
	else
		local _info="$red 1844已关闭$none"
	fi
	_opt=''
	while :; do
		echo
		echo -e "$yellow 1. $none 1849开启 广告拦截"
		echo
		echo -e "$yellow 2. $none 1851关闭 广告拦截"
		echo
		...
		echo
		read -p "$(echo -e "1859请选择 [${magenta}1-2$none]:")" _opt
		if [[ -z $_opt ]]; then
			error
		else
			case $_opt in
			1)
				if [[ $ban_ad ]]; then
					echo
					echo -e " 1867大胸弟...难不成你没有看到 (当前广告拦截状态: $_info) 这个帅帅的提示么.....还开启个鸡鸡哦"
					echo
				else
					echo
					echo
					echo -e "$yellow 1872 广告拦截 = $cyan开启$none"
					echo "----------------------------------------------------------------"
					echo
					pause
					backup_config +ad
					...
					echo
					echo
					echo -e "$green 1881 广告拦截已开启...如果出现异常..那就关闭它咯$none"
					echo
				fi
				break
				;;
			2)
				if [[ $ban_ad ]]; then
					echo
					echo
					echo -e "$yellow 1890 广告拦截 = $cyan关闭$none"
					echo "----------------------------------------------------------------"
					echo
					pause
					backup_config -ad
					...
					echo
					echo
					echo -e "$red 1899 广告拦截已关闭...不过你也可以随时重新开启 ...只要你喜欢$none"
					echo
				else
					echo
					echo -e " 1903 大胸弟...难不成你没有看到 (当前广告拦截状态: $_info) 这个帅帅的提示么.....还关闭个鸡鸡哦"
					echo
				fi
				break
				;;
...
change_v2ray_alterId() {
	echo
	while :; do
		echo -e "1919请输入 ${yellow}alterId${none} 的数值 [${magenta}0-65535$none]"
		read -p "$(echo -e "(1920当前数值是: ${cyan}$alterId$none):") " new_alterId
		[[ -z $new_alterId ]] && error && continue
		case $new_alterId in
		$alterId)
			echo
			echo -e "1925 大佬...跟 当前 alterId 一毛一样啊...修改个鸡鸡哦 "
			echo
			error
			;;
...
custom_uuid() {
	echo
	while :; do
		echo -e "1954请输入$yello自定义的 UUID$none...(${cyan}UUID 格式一定要对!!!$none)"
		read -p "$(echo -e "(1955当前 UUID: ${cyan}${v2ray_id}$none)"): " myuuid
		[ -z "$myuuid" ] && error && continue
		case $myuuid in
		$v2ray_id)
			echo
			echo -e " 1960大佬...跟 当前 UUID 一毛一样啊...修改个鸡鸡哦 "
			echo
			error
			;;
		*[/$]* | *\&*)
			echo
			echo -e " 1966由于这个脚本太辣鸡了..所以 UUID 不能包含$red / $none或$red $ $none或$red & $none这三个符号.... "
			echo
			error
			;;
		*)
			...
			echo
			echo -e " 1975如果 UUID 格式不正确.. V2Ray 会跪...使用$cyan v2ray reuuid$none 复活"
			echo "----------------------------------------------------------------"
			echo
			pause
			uuid=$myuuid
...
v2ray_service() {
	while :; do
		echo
		echo -e "$yellow 1. $none 1994启动 V2Ray"
		echo
		...
		echo
		read -p "$(echo -e "2004 请选择 [${magenta}1-5$none]:")" _opt
		if [[ -z $_opt ]]; then
			error
		else
			case $_opt in
...
start_v2ray() {
	if [[ $v2ray_pid ]]; then
		echo
		echo -e "${green} 2039V2Ray 正在运行...无需再启动$none"
		echo
	else
		...
		if [[ $? -ne 0 ]]; then
			echo
			echo -e "${red} 2047V2Ray 启动失败！$none"
			echo
		else
			echo
			echo -e "${green} 2051V2Ray 已启动$none"
			echo
		fi
...
stop_v2ray() {
	if [[ $v2ray_pid ]]; then
		...
		echo
		echo -e "${green} 2062V2Ray 已停止$none"
		echo
	else
		echo
		echo -e "${red} 2066V2Ray 没有在运行$none"
		echo
	fi
}
restart_v2ray() {
	...
	if [[ $? -ne 0 ]]; then
		echo
		echo -e "${red} 2075V2Ray 重启失败！$none"
		echo
	else
		echo
		echo -e "${green} 2079V2Ray 重启完成 $none"
		echo
	fi
}
view_v2ray_log() {
	echo
	echo -e "$green 2085按 Ctrl + C 即可退出...$none"
	echo
	tail -f /var/log/v2ray/access.log
}
view_v2ray_error_log() {
	echo
	echo -e "$green 2091按 Ctrl + C 即可退出...$none"
	echo
...
download_v2ray_config() {
	while :; do
		echo
		echo -e "$yellow 1. $none 2098直接下载 V2Ray 客户端配置文件(仅支持 Xshell)"
		echo
		...
		echo
		read -p "$(echo -e "2106请选择 [${magenta}1-4$none]:")" other_opt
		if [[ -z $other_opt ]]; then
			error
		else
			case $other_opt in
...
get_v2ray_config() {
	...
	echo
	while :; do
		read -p "$(echo -e "2140不要BB...哥就是在使用 Xshell [${magenta}Y$none]:")" is_xshell
		if [[ -z $is_xshell ]]; then
			error
		else
			if [[ $is_xshell == [yY] ]]; then
				echo
				echo "2146开始下载....请选择 V2Ray 客户端配置文件保存位置"
				echo
				...
				echo
				echo
				echo -e "$green 2154下载完成咯...$none"
				echo
				# echo -e "$yellow 解压密码 = ${cyan}233blog.com$none"
				# echo
				echo -e "$yellow 2158SOCKS 监听端口 = ${cyan}2333${none}"
				echo
				echo -e "${yellow} 2160HTTP 监听端口 = ${cyan}6666$none"
				echo
				echo "2162V2Ray 客户端使用教程: https://v2ray6.com/post/4/"
				echo
				break
			else
				error
...
get_v2ray_config_link() {
	...
	if [[ $link ]]; then
		...
		echo
		echo -e "$red 2197 哎呀呀呀...出错咯...请重试$none"
		echo
	fi
}
create_v2ray_config_text() {
	...
	echo
	echo
	echo "---------- 2207 V2Ray 配置信息 -------------"
	if [[ $v2ray_transport == [45] ]]; then
		if [[ ! $caddy ]]; then
			echo
			echo " 2211警告！请自行配置 TLS...教程: https://v2ray6.com/post/3/"
		fi
		echo
...
get_v2ray_config_info_link() {
	echo
	echo -e "$green 2269 正在生成链接.... 稍等片刻即可....$none"
	echo
	...
	if [[ $link ]]; then
		...
		echo
		echo -e "$red 2288 哎呀呀呀...出错咯...请重试$none"
		echo
	fi
...
get_v2ray_config_qr_link() {
	...
	echo
	echo -e "$green 2298 正在生成链接.... 稍等片刻即可....$none"
	echo
...
update() {
	while :; do
		echo
		echo -e "$yellow 1. $none 2496 更新 V2Ray 主程序"
		echo
		echo -e "$yellow 2. $none 2498 更新 V2Ray 管理脚本"
		echo
		read -p "$(echo -e "2500 请选择 [${magenta}1-2$none]:")" _opt
		if [[ -z $_opt ]]; then
			error
		else
			case $_opt in
...
update_v2ray.sh() {
	...
	if [[ ! $latest_version ]]; then
		echo
		echo -e " $red 2534获取 V2Ray 最新版本失败!!!$none"
		echo
		echo -e " 2536请尝试执行如下命令: $green echo 'nameserver 8.8.8.8' >/etc/resolv.conf $none"
		echo
		echo " 2538 然后再继续...."
		echo
		exit 1
	...
	if [[ $latest_version == $_version ]]; then
		echo
		echo -e "$green 2545木有发现新版本 $none"
		echo
	else
		echo
		echo -e " $green 2549咦...发现新版本耶....正在拼命更新.......$none"
		echo
		...
		chmod +x $_v2ray_sh
		echo
		echo -e "$green 2556更新成功啦...当前 V2Ray 管理脚本 版本: ${cyan}$latest_version$none"
		echo
	fi
...
pause() {
	...
	read -rsp "$(echo -e "2696 按$green Enter 回车键 $none继续....或按$red Ctrl + C $none取消.")" -d $'\n'
	echo
}
do_service() {
	if [[ $systemd ]]; then
...
_help() {
	echo
	echo "2708........... V2Ray 管理脚本帮助信息 by v2ray6.com .........."
	echo -e "
...
menu() {
	...
	while :; do
		echo
		echo "2749........... V2Ray 管理脚本 $_version by v2ray6.com .........."
		echo
...
v | version)
	echo
	echo -e " 2963当前 V2Ray 版本: ${green}$v2ray_ver$none  /  当前 V2Ray 管理脚本版本: ${cyan}$_version$none"
	echo
	;;
bbr)
	other
